...
Copy and Move now name their collisions and ask (see [design](design.md)), but five paths still resolve or destroy without saying so, or refuse a collision that is not one. None loses data silently today; each is a stated-behaviour gap rather than a safety hole.

- **Import has no idea a row can already be where it is going.** Rename calls that a no-op; `import_analysis` only asks `primary.exists() || planned_destinations.contains(primary)`, so a source whose destination resolves to *its own path* is reported as a collision. Measured with the default `{year}\{created}` structure over a library Diffractor already organised, importing the selection back into the same library: Block Run refuses the whole import and states "1 destination already exists", Skip drops the row, Replace plans a move of the file onto itself (`MoveFileEx` no-ops it, `CopyFile` fails it), and **Auto-rename plans `IMG (2).jpg` for a file that needed nothing** — with Move that renames a correctly-filed photo, with Copy it duplicates it. The fix is a `path_in == path_out` no-op test in `import_analysis`, ahead of the collision test.
- **Import with Move also calls a destination it frees itself a collision.** The narrower half of the same missing concept, and the one Rename closed in this release: a source already inside the destination tree whose capture date moves it to another folder frees its old path, and a second source of the same name targeting that path is refused under Block Run. Reproduced with a `{year}` library. Closing it needs both halves Rename needed — a vacated-path set in the analysis *and* freeing the occupant in `import_copy`, which has to interleave with that function's `.original` backups, per-row Replace snapshots and rollback. Copy, Sync and Convert are unaffected because they never move a source away.
