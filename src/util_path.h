...
namespace df
{
	...
	constexpr std::string_view::size_type find_ext(const std::string_view path)
	{
	...
	public:
		static bool is_guid_path(const std::string_view s)
		{
			if (s.size() < 8) return false;
			return s[0] == L':' && s[1] == L':' && s[2] == L'{';
		}

		static bool is_device_path(const std::string_view s)
		{
			return str::starts(s, "\\Device\\");
		}
		...
		static bool is_drive(const std::string_view s)
		{
		...
		static bool is_computer(const std::string_view path)
		{
			return str::icmp(path, "Computer") == 0;
		}
	...
	public:
		...
		file_path(const folder_path folder, const std::string_view name_in,
		...
		file_path(const folder_path folder, const std::string_view name) : _folder(folder), _name(str::cache(name))
		{
			assert_true(is_valid());
		}
		...
		void un_pack(const std::string_view path)
		{
		...
		static bool is_original(const std::string_view name)
		{
			return str::contains(name, ".original.");
		}
		...
		static bool is_link(const std::string_view path)
		{
			return str::ends(path, ".lnk");
		}

		std::string_view file_name_without_extension() const
		{
			const std::string_view sv = _name;
			return sv.substr(0, find_ext(sv));
		}

		file_path extension(const std::string_view ext) const
		{
	...
	inline file_path folder_path::combine_file_ext(const std::string_view part, const std::string_view ext) const
	{
	...
	file_path probe_data_file(std::string_view file_name);
}

template <>
struct std::formatter<df::folder_path, char> : std::formatter<std::string_view, char>
{
	auto format(const df::folder_path& p, std::format_context& ctx) const
	{
		return std::formatter<std::string_view, char>::format(p.text().sv(), ctx);
	}
};

template <>
struct std::formatter<df::file_path, char> : std::formatter<std::string_view, char>
{
	auto format(const df::file_path& p, std::format_context& ctx) const
	{
		const auto packed = p.pack();
		return std::formatter<std::string_view, char>::format(std::string_view{packed}, ctx);
	}
};
