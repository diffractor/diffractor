...
public:
	temp_files() : _folder(platform::temp_folder().combine(str::format(u8"diffractor-{}", platform::tick_count())))
	{
	}
	...
	df::file_path next_path(const std::u8string_view ext = u8".jpg")
	{
		if (!_folder.exists())
		{
...
static df::index_file_item make_index_file_info(const df::date_t date)
{
	...
	result.ft = files::file_type_from_name(u8"test.jpg");
	result.safe_ps();
	return result;
}
...
void test_view::test_element::render(ui::draw_context& dc, const pointi element_offset) const
{
	...
	{
		message = str::format(u8"{:8} milliseconds", _time);
	}
...
{
	static constexpr std::u8string_view titles[] = {u8"Name", u8"Success", u8"Message"};
                         ...
                         const std::u8string_view name = {}, const std::u8string_view message = {})
{
	...
	{
		throw test_assert_exception(str::format(u8"{} - {}: expected '{}', got '{}'", message, name, expected, actual));
	}
}
                             ...
                             const std::u8string_view name = {}, const std::u8string_view message = {})
{
	if (str::icmp(actual, expected) == 0)
	{
		throw test_assert_exception(str::format(u8"{} - {}: expected '{}' not to equal '{}'", message, name, expected,
		                                        actual));
	}
}
...
void assert_metadata(const prop::item_metadata& expected, const prop::item_metadata& actual,
                     const std::u8string_view message = {})
{
	assert_equal(expected.album, actual.album, u8"album", message);
	assert_equal(expected.album_artist, actual.album_artist, u8"album_artist", message);
	assert_equal(expected.artist, actual.artist, u8"artist", message);
	assert_equal(expected.audio_codec, actual.audio_codec, u8"audio_codec", message);
	assert_equal(expected.audio_sample_rate, actual.audio_sample_rate, u8"audio_sample_rate", message);
	assert_equal(expected.audio_sample_type, actual.audio_sample_type, u8"audio_sample_type", message);
	assert_equal(expected.audio_channels, actual.audio_channels, u8"audio_channels", message);
	assert_equal(expected.bitrate, actual.bitrate, u8"bitrate", message);
	assert_equal(expected.camera_manufacturer, actual.camera_manufacturer, u8"camera_manufacturer", message);
	assert_equal(expected.camera_model, actual.camera_model, u8"camera_model", message);
	assert_equal(expected.comment, actual.comment, u8"comment", message);
	assert_equal(expected.composer, actual.composer, u8"composer", message);
	assert_equal(expected.coordinate, actual.coordinate, u8"coordinate", message);
	assert_equal(expected.copyright_creator, actual.copyright_creator, u8"copyright_creator", message);
	assert_equal(expected.copyright_credit, actual.copyright_credit, u8"copyright_credit", message);
	assert_equal(expected.copyright_notice, actual.copyright_notice, u8"copyright_notice", message);
	assert_equal(expected.copyright_source, actual.copyright_source, u8"copyright_source", message);
	assert_equal(expected.copyright_url, actual.copyright_url, u8"copyright_url", message);
	assert_equal(expected.created_digitized, actual.created_digitized, u8"created_digitized", message);
	assert_equal(expected.created_exif, actual.created_exif, u8"created_exif", message);
	assert_equal(expected.created_utc, actual.created_utc, u8"created_utc", message);
	assert_equal(expected.description, actual.description, u8"description", message);
	assert_equal(expected.width, actual.width, u8"width", message);
	assert_equal(expected.height, actual.height, u8"height", message);
	assert_equal(expected.disk, actual.disk, u8"disk", message);
	assert_equal(expected.duration, actual.duration, u8"duration", message);
	assert_equal(expected.encoder, actual.encoder, u8"encoder", message);
	assert_equal(expected.episode, actual.episode, u8"episode", message);
	assert_equal(prop::format_exposure(expected.exposure_time), prop::format_exposure(actual.exposure_time),
	             u8"exposure_time", message);
	assert_equal(prop::format_f_num(expected.f_number), prop::format_f_num(actual.f_number), u8"f_number", message);
	assert_equal(expected.file_name, actual.file_name, u8"file_name", message);
	assert_equal(expected.focal_length, actual.focal_length, u8"focal_length", message);
	assert_equal(expected.focal_length_35mm_equivalent, actual.focal_length_35mm_equivalent,
	             u8"focal_length_35mm_equivalent", message);
	assert_equal(expected.genre, actual.genre, u8"genre", message);
	assert_equal(expected.iso_speed, actual.iso_speed, u8"iso_speed", message);
	assert_equal(expected.lens, actual.lens, u8"lens", message);
	assert_equal(expected.location_place, actual.location_place, u8"location_city", message);
	assert_equal(expected.location_country, actual.location_country, u8"location_country", message);
	assert_equal(expected.location_state, actual.location_state, u8"location_state", message);
	assert_equal(expected.orientation, actual.orientation, u8"orientation", message);
	assert_equal(expected.performer, actual.performer, u8"performer", message);
	//assert_equal(expected.pixel_format, actual.pixel_format, u8"pixel_format", message);
	assert_equal(expected.publisher, actual.publisher, u8"publisher", message);
	assert_equal(expected.rating, actual.rating, u8"rating", message);
	assert_equal(expected.audio_sample_rate, actual.audio_sample_rate, u8"sample_rate", message);
	assert_equal(expected.season, actual.season, u8"season", message);
	assert_equal(expected.show, actual.show, u8"show", message);
	assert_equal(expected.synopsis, actual.synopsis, u8"synopsis", message);
	assert_equal(expected.tags, actual.tags, u8"tags", message);
	assert_equal(expected.title, actual.title, u8"title", message);
	assert_equal(expected.track, actual.track, u8"track", message);
	assert_equal(expected.video_codec, actual.video_codec, u8"video_codec", message);
	assert_equal(expected.year, actual.year, u8"year", message);
}
...
{
	files ff;
	const auto image_md = ff_scan_file(ff, test_files_folder.combine_file(u8"Test.jpg"));
	const auto audio_md = ff_scan_file(ff, test_files_folder.combine_file(u8"Colorblind.mp3"));

	assert_equal(u8"2012\\2012-09-14", replace_tokens(u8"{year}\\{created}"s, image_md.to_props()));
	assert_equal(u8"Counting Crows\\This Desert Life", replace_tokens(u8"{artist}\\{album}"s, audio_md.to_props()));
}

static void should_scan_jpeg()
{
	const auto load_path = test_files_folder.combine_file(u8"Test.jpg");
	...
	assert_metadata(expected_exif, *extract_properties(load_path, metadata_type::EXIF), u8"EXIF");
	...
	assert_metadata(expected_iptc, *extract_properties(load_path, metadata_type::IPTC), u8"IPTC");
	...
	assert_metadata(expected_xmp, *actual_xmp, u8"XMP");

	const auto actual_all = extract_properties(load_path, metadata_type::ALL);
	assert_metadata(*expected_test_jpg(), *actual_all, u8"all");
}

static void should_parse_xmp()
{
	const auto load_path = test_files_folder.combine_file(u8"IMG_0604.xmp");

	prop::item_metadata actual;
	metadata_xmp::parse(actual, load_path);

	assert_equal(u8"Flower", actual.title);
	assert_equal(u8"Blomst", actual.tags);
	assert_equal(u8"Følfod ( Tussilago farfara ) Lægeurt", actual.description);
	assert_equal(u8"Frank Aalestrup www.fdaa.dk", actual.copyright_notice);
	assert_equal(u8"\"Frank Aalestrup.\nwww.fdaa.dk\"", actual.copyright_creator);
	assert_equal(u8"EF100mm f/2.8L Macro IS USM", actual.lens);
	assert_equal({56.19283, 9.88415}, actual.coordinate);
	assert_equal(u8"Canon", actual.camera_manufacturer);
	assert_equal(u8"Canon EOS 50D", actual.camera_model);
	assert_equal(u8"IMG_0604.CR2", actual.raw_file_name);
	//assert_equal(u8"Toustrup Mark", actual.location_city);
	//assert_equal(u8"New York", actual.location_state);
	assert_equal(u8"Denmark", actual.location_country);
}

static void should_merge_xmp_sidecar()
{
	const auto cr2_path = test_files_folder.combine_file(u8"Gherkin.CR2");
	const auto xmp_path = test_files_folder.combine_file(u8"Gherkin.XMP");
	...
	assert_equal(u8"Canon", actual->camera_manufacturer);
	assert_equal(u8"United Kingdom", actual->location_country);
	assert_equal(u8"\xA9 Mark Ridgwell", actual->copyright_notice);
	assert_equal(u8"\"Mark Ridgwell\"", actual->copyright_creator);
}
...
{
	const auto load_path = test_files_folder.combine_file(u8"Colorblind.mp3");
	...
	assert_metadata(expected, *actual.to_props(), u8"Colorblind.mp3");

	const auto load_path2 = test_files_folder.combine_file(u8"Games Without Frontiers.mp3");
	...
	assert_metadata(expected2, *actual2.to_props(), u8"Games Without Frontiers.mp3");

	const auto load_path3 = test_files_folder.combine_file(u8"Is It Any Wonder.mp3");
	...
	assert_metadata(expected3, *actual3.to_props(), u8"Is It Any Wonder.mp3");
}

static void should_scan_mp4()
{
	const auto load_path = test_files_folder.combine_file(u8"gizmo.mp4");
	...
	assert_metadata(expected, *actual.to_props(), u8"gizmo.mp4");

	const auto load_path2 = test_files_folder.combine_file(u8"This Year's Love.m4a");
	const auto actual2 = ff_scan_file(ff, load_path2);
	...
	assert_metadata(expected2, *actual2.to_props(), u8"This Year's Love.m4a");
	assert_equal(true, is_empty(actual2.thumbnail_image), u8"m4a scan thumbnail");

	const auto loaded = ff_scan_and_load_thumb(ff, load_path2);
	assert_equal(false, loaded.success && is_valid(loaded.thumbnail_surface), u8"m4a load thumbnail");
}

static void should_scan_mov()
{
	const auto load_path = test_files_folder.combine_file(u8"ipod.mov");
	...
	assert_metadata(expected, *actual.to_props(), u8"ipod.mov");

	const auto load_path2 = test_files_folder.combine_file(u8"StPauls.MOV");
	const auto actual2 = ff_scan_file(ff, load_path2);
	...
	assert_metadata(expected2, *actual2.to_props(), u8"StPauls.MOV");
}

static void should_scan_avi()
{
	const auto load_path = test_files_folder.combine_file(u8"Byzantium.avi");
	...
	assert_metadata(expected, *actual.to_props(), u8"Byzantium.avi");
}

static void should_parse_searches()
{
	const auto date_filter = df::search_t::parse(u8"2012-09-14");
	const df::search_matcher matcher(date_filter);

	df::index_file_item info;
	info.ft = files::file_type_from_name(u8"test.jpg");
	info.safe_ps()->created_exif = df::date_t(2012, 9, 14);
	assert_equal(true, matcher.match_item({}, info).is_match(), u8"date");

	info.safe_ps()->created_exif = df::date_t(2012, 9, 13);
	assert_equal(false, matcher.match_item({}, info).is_match(), u8"date");
}
...
std::u8string_view detect_xmp_sidecar(const df::file_path path)
{
	...
	const auto xmp_path = path.extension(u8".xmp");
...
static void should_update_rating(const std::u8string_view name)
{
...

	{
		...
		assert_equal(3, ps->rating, u8"rating");
	}
...

	{
		const auto actual_scanned = ff_scan_file(ff, save_path, detect_xmp_sidecar(save_path));
		const auto ps = actual_scanned.to_props();
		assert_equal(0, ps->rating, u8"rating");
	}
}

static void should_update_exif_rating()
{
	const auto load_path = test_files_folder.combine_file(u8"exif-rating.jpg");
	const auto save_path = _temps.next_path(u8".jpg");
...

	{
		...
		const auto ps = actual_scanned.to_props();
		assert_equal(3, ps->rating, u8"to_props");
		...
		assert_equal(3, actual_xmp->rating, u8"XMP");
		assert_equal(3, actual_exif->rating, u8"exif");
		//assert_equal(3, actual_iptc.rating, u8"IPC");
	}
...

	{
		...
		assert_equal(0, actual_xmp->rating, u8"XMP");
		assert_equal(0, actual_exif->rating, u8"exif");
		assert_equal(0, actual_iptc->rating, u8"IPC");
	}
}
...
static void should_update_formatted_text()
{
...

	{
		const auto actual_scanned = ff_scan_file(ff, save_path);
		const auto ps = actual_scanned.to_props();
		assert_equal(desc_text, ps->description, u8"to_props");

		const auto actual_exif = extract_properties(save_path, metadata_type::EXIF);
		const auto actual_iptc = extract_properties(save_path, metadata_type::IPTC);
		const auto actual_xmp = extract_properties(save_path, metadata_type::XMP);

		assert_equal(desc_text, actual_xmp->description, u8"XMP");
		assert_equal(desc_text, actual_exif->description, u8"exif");
		assert_equal(desc_text, actual_iptc->description, u8"IPC");
	}
}
...
static void should_update_metadata(const std::u8string_view name)
{
	...
	const auto tags_to_add = tag_set(u8"key1 key2 key3");

	metadata_edits metadata_edits;
	metadata_edits.add_tags = tags_to_add;
	metadata_edits.copyright_notice = u8"Copyright xx";
	metadata_edits.rating = 3;
	metadata_edits.title = u8"Title xx";
	metadata_edits.description = u8"Description xx";
...
static void should_add_remove_tags(const std::u8string_view name)
{
	...
	const auto tags_to_add = tag_set(u8"Максим yyyy zzzz");
	const auto tags_to_remove = tag_set(u8"Максим zzzz");
	...
	assert_metadata(*expected, *actual, u8"added");
	...
	assert_metadata(*expected, *actual2, u8"removed");
}
...
{
	const auto save_path = _temps.next_path(u8".jpg");
	const auto load_path = test_files_folder.combine_file(u8"IMG_9340.jpg");
	const auto coordinate = gps_coordinate(40.71417, -74.00611);
	...
	assert_equal(u8"Big Apple", actual_iptc->location_place, u8"IPTC");
	assert_equal(u8"New York", actual_iptc->location_state, u8"IPTC");
	assert_equal(u8"USA", actual_iptc->location_country, u8"IPTC");
	assert_equal({40.71417, -74.00611}, actual_exif->coordinate, u8"EXIF");

	assert_equal(u8"Big Apple", actual_xmp->location_place, u8"XMP");
	assert_equal(u8"New York", actual_xmp->location_state, u8"XMP");
	assert_equal(u8"USA", actual_xmp->location_country, u8"XMP");

	// TODO
	//AssertEqual(u8"40.71417", actual_xmp, Property::Latitude, u8"XMP");
	//AssertEqual(u8"-74.00611", actual_xmp, Property::Longitude, u8"XMP");
}
...
static void should_handle_international_characters()
{
	const auto save_path = _temps.next_path(u8".jpg");
	const auto load_path = test_files_folder.combine_file(u8"test.jpg");
	const auto* description = u8"In vollen Zügen genießen";

	tag_set tags;
	tags.add_one(u8"In vollen Zügen genießen");
	tags.add_one(u8"Nældens takvinge");
	tags.add_one(u8"Žižkov");
	...
	edits.remove_tags = tag_set(u8"key1 key2 key3");
	edits.add_tags = tags;
	...
	assert_equal(description, actual_xmp->description, u8"XMP");
	assert_equal(description, actual_exif->description, u8"EXIF");
	assert_equal(description, actual_iptc->description, u8"IPC");

	const auto expected_tags = tags.to_string();
	assert_equal(expected_tags, actual_xmp->tags, u8"XMP Tags");
	//AssertEqual(expectedTags, actual_exif.Combine(Property::Tag), u8"Exif Tags");
	assert_equal(expected_tags, actual_iptc->tags, u8"IPTC Tags");

	const auto sr = ff_scan_file(ff, save_path);
	const auto ps = sr.to_props();
	assert_equal(description, ps->description);
	assert_equal(expected_tags, ps->tags, u8"Tags");
}

static void should_scan_raw()
{
	const auto load_path = test_files_folder.combine(u8"raw").combine_file(u8"Screws.CR2");
	...
	assert_metadata(expected, *actual, u8"Screws.CR2");
}
...
static void should_convert_raw_to_jpeg()
{
	...
	//assert_metadata(*expected, *actual);
	assert_equal(expected->tags, actual->tags, u8"tags");
	assert_equal(expected->title, actual->title, u8"title");
	assert_equal(expected->description, actual->description, u8"description");
	assert_equal(expected->width, actual->width, u8"width");
	assert_equal(expected->height, actual->height, u8"height");
}

static void should_parse_facebook_json()
{
	const auto path_status = test_files_folder.combine_file(u8"facebook.state.json");
	const auto status = df::util::json::json_from_file(path_status);

	assert_equal(u8"804051981"sv, status[u8"id"].GetString(), u8"Id");

	const auto path_albums = test_files_folder.combine_file(u8"facebook.albums.json");
	auto json = df::util::json::json_from_file(path_albums);
	auto& albums = json[u8"data"];

	assert_equal(19u, albums.Size(), u8"data");
	assert_equal(u8"10150677908351982"sv, albums[0][u8"id"].GetString(), u8"id");
	assert_equal(u8"Wall Photos"sv, albums[0][u8"name"].GetString(), u8"name");
	assert_equal(false, albums[0][u8"can_upload"].GetBool(), u8"can_upload");
}
...
static void should_rotate()
{
...

	{
		...
		const auto expected = extract_properties(test_files_folder.combine_file(u8"Test90.jpg"));
		const auto actual = extract_properties(save_path);
...

	{
		...
		const auto save_path = _temps.next_path();
		const auto load_path = test_files_folder.combine_file(u8"exif-rotated.jpg");
		const auto loaded = ff.load(load_path, false);
		...
		const auto actual_exif = extract_properties(save_path, metadata_type::EXIF);
		assert_equal(ui::orientation::top_left, actual_exif->orientation, u8"orientation");
	}

	{
		// PNG
		const auto save_path = _temps.next_path(u8".png");
		const auto load_path = test_files_folder.combine_file(u8"engine.png");
		const auto loaded = ff.load(load_path, false);
		...
		assert_equal(loaded.i->height(), updated.i->width(), u8"png width");
		assert_equal(loaded.i->width(), updated.i->height(), u8"png width");
	}

	{
		// WEBP
		const auto save_path = _temps.next_path(u8".webp");
		const auto load_path = test_files_folder.combine_file(u8"lake.webp");
		const auto loaded = ff.load(load_path, false);
		...
		const auto updated = ff.load(save_path, false);
		assert_equal(false, updated.is_empty(), u8"webp result empty");
		assert_equal(loaded.i->height(), updated.i->width(), u8"webp width");
		assert_equal(loaded.i->width(), updated.i->height(), u8"webp width");
	}
}
...
{
	const auto save_path = _temps.next_path();
	const auto load_path = test_files_folder.combine_file(u8"Lossless0.jpg");
	...
	const auto expected = extract_properties(test_files_folder.combine_file(u8"Lossless90.jpg"));
	const auto actual = extract_properties(save_path);
...
static void should_preserve_orientation()
{
	...
	assert_equal(loaded_orientation, encoded_jpg->orientation(), u8"orientation for created jpg");

	const auto encoded_png = ff.surface_to_image(surface, {}, {}, ui::image_format::PNG);
	assert_equal(loaded_orientation, encoded_png->orientation(), u8"orientation for created png");

	const auto encoded_webp = ff.surface_to_image(surface, {}, {}, ui::image_format::WEBP);
	assert_equal(loaded_orientation, encoded_webp->orientation(), u8"orientation for created webp");

	const auto loaded_jpg = load_image_file(encoded_jpg->data()); // will cause re-parse
	assert_equal(loaded_orientation, loaded_jpg->orientation(), u8"orientation for re-loaded jpg");

	const auto loaded_png = load_image_file(encoded_png->data()); // will cause re-parse
	assert_equal(loaded_orientation, loaded_png->orientation(), u8"orientation for re-loaded png");

	const auto loaded_webp = load_image_file(encoded_webp->data()); // will cause re-parse
	assert_equal(loaded_orientation, loaded_webp->orientation(), u8"orientation for re-loaded webp");
}
...
static void should_store_thumbnails()
{
	...
	const auto file_path = test_files_folder.combine_file(u8"Test.jpg");
	...
	assert_equal(i->thumbnail(), thumb.thumb, u8"local loaded thumb");
}
...
static void should_store_item_properties()
{
	...
	assert_metadata(*md, *item.metadata, u8"index");
	assert_equal(crc32c_expected, item.crc32c, u8"index crc32");
	assert_equal(static_cast<int>(media_pos), static_cast<int>(item.metadata->media_position),
	             u8"index media position");
	assert_equal(md->orientation, item.metadata->orientation, u8"index orientation");

	const auto reloaded_crc = platform::file_crc32(file_path);
	assert_equal(reloaded_crc, item.crc32c, u8"platform::file_crc32 crc32");
}
...
static void should_pack_item_properties()
{
	...
	assert_metadata(*md, *unpacked, u8"index");
	assert_equal(md->orientation, unpacked->orientation, u8"index orientation");
}
...
static void should_store_webservice_results()
{
	...
	const auto* const key = u8"key   xxxxxxxxxx";
	const auto* const value = long_text;
	db.web_service_cache(key, value);
	const auto result = db.web_service_cache(key);

	assert_equal(result, value, u8"web_service_cache");
}
...
static void build_index(index_state& index, database& db)
{
	...
	assert_equal(expected_cached_item_count, index.stats.media_item_count, u8"cached item count");
}
...
static void should_index(test_view::shared_test_context& stc)
{
	...
	assert_equal(expected_cached_item_count, stc.index.stats.media_item_count, u8"cached item count");

	const auto expected_md = expected_test_jpg();
	expected_md->file_name = u8"Test.jpg"_c;
	// Embedded values
	assert_metadata(*expected_md, *metadata_from_cache(stc.index, test_files_folder.combine_file(u8"Test.jpg")),
	                u8"Test.jpg");

	// Sidecar values
	const auto actual = metadata_from_cache(stc.index, test_files_folder.combine_file(u8"Gherkin.CR2"));
	assert_equal(u8"Canon", actual->camera_manufacturer, u8"camera_manufacturer");
	assert_equal(u8"United Kingdom", actual->location_country, u8"location_country");
	assert_equal(u8"\xA9 Mark Ridgwell", actual->copyright_notice, u8"copyright_notice");
	assert_equal(u8"\"Mark Ridgwell\"", actual->copyright_creator, u8"copyright_creator");
}
...
public:
	prop_test()
	{
		_f.ft = files::file_type_from_name(u8"test.jpg");
	}
...
static void should_match_terms()
{
	prop_test().tag(u8"aaa")
	           ...
	           .is_not_match(u8"-#aaa")
	           .is_not_match(u8"!#aaa")
	           .is_match(u8"#aaa or #bbb")
	           .is_not_match(u8"#aaa and #bbb")
	           .is_not_match(u8"#aaa #bbb");

	prop_test().tag(u8"bbb")
	           .is_not_match(u8"#aaa")
	           .is_match(u8"#bbb")
	           .is_match(u8"#aaa or #bbb");

	prop_test().tag(u8"'aa bb'")
	           .is_not_match(u8"#aaa")
	           .is_not_match(u8"#bb")
	           .is_match(u8"#'aa bb'");
	...
	prop_test().rate(4)
	           ...
	           .is_match(u8"3 | 4")
	           .is_match(u8"3 or 4")
	           .is_not_match(u8"3 and 4");

	prop_test().desc(u8"one two three")
	           .is_match(u8"two")
	           .is_match(u8"one two")
	           .is_match(u8"two three")
	           .is_match(u8"'two three'")
	           .is_not_match(u8"'one three'");

	prop_test().date(1999, 12, 27)
	           .is_match(u8"age:5")
	           .is_match(u8"age:10")
	           .is_not_match(u8"age:1")
	           .is_not_match(u8"age:2");

	prop_test().date(2000, 1, 1)
	           .is_match(u8"age:1")
	           .is_match(u8"age:5")
	           .is_not_match(u8"-age:5")
	           .is_not_match(u8"!age:5");

	prop_test().tag(u8"aaa").date(2000, 1, 1)
	           .is_match(u8"#aaa age:1")
	           .is_match(u8"#aaa created:2000-jan")
	           .is_not_match(u8"created:2000-feb")
	           .is_not_match(u8"#bbb created:2000-jan");

	// Metadata and file date. Should use meta
	prop_test().file_created_date(2000, 1, 1).date(1999, 5, 25)
	           .is_not_match(u8"created:9")
	           .is_not_match(u8"created:2000-jan")
	           .is_match(u8"created:1999-may");

	// file date but not metadata
	prop_test().file_created_date(2000, 1, 1)
	           .is_match(u8"age:10")
	           .is_match(u8"created:2000-jan");
}
...
static void should_match_date(const std::u8string_view query, const df::date_t d)
{
	...
	props_without_val.ft = files::file_type_from_name(u8"test.jpg");
	props_without_val.file_modified = df::date_t(1972, 5, 25);
	props_without_val.safe_ps();

	df::index_file_item props_with_val;
	props_with_val.ft = files::file_type_from_name(u8"test.jpg");
                  ...
                  bool is_recursive, const std::u8string_view wildcard)
{
	...
	assert_equal(display, sel.str(), u8"item_selector.str");
	assert_equal(folder, sel.folder().text(), u8"item_selector.folder");
	assert_equal(is_recursive, sel.is_recursive(), u8"item_selector.is_recursive");
	assert_equal(wildcard, sel.wildcard(), u8"item_selector.wildcard");
}
...
{
	assert_parse(u8"c:\\", u8"c:\\", false, u8"*.*");
	assert_parse(u8"c:\\**", u8"c:\\", true, u8"*.*");
	assert_parse(u8"c:\\**\\", u8"c:\\**", u8"c:\\", true, u8"*.*");
	assert_parse(u8"c:/**/", u8"c:\\**", u8"c:\\", true, u8"*.*");
	assert_parse(u8"c:\\*.jpg", u8"c:\\", false, u8"*.jpg");
	assert_parse(u8"c:\\temp\\*.jpg", u8"c:\\temp", false, u8"*.jpg");
	assert_parse(u8R"(c:\temp\**\*.jpg)", u8"c:\\temp", true, u8"*.jpg");
	assert_parse(u8"c:\\temp\\***.jpg", u8"c:\\temp", false, u8"***.jpg");
	assert_parse(u8"c:\\temp\\?x.jpg", u8"c:\\temp", false, u8"?x.jpg");
}
...
static void should_select_files()
{
	...
	assert_equal(true, contains(platform::select_files(recursive, true), u8"Screws.CR2"), u8"files from sub folders");
	assert_equal(true, contains(platform::select_files(recursive, true), u8"Cube.png"), u8"files from sub folders");
	assert_equal(true, contains(platform::select_files(recursive, true), u8"Test.jpg"), u8"files from sub folders");
	assert_equal(false, contains(platform::select_folders(recursive, true), u8"raw"), u8"no folders");

	const auto not_recursive = df::item_selector(test_files_folder, false);
	assert_equal(false, contains(platform::select_files(not_recursive, true), u8"Screws.CR2"),
	             u8"no files from sub folder");
	assert_equal(true, contains(platform::select_files(not_recursive, true), u8"Cube.png"), u8"files from sub folders");
	assert_equal(true, contains(platform::select_files(not_recursive, true), u8"Test.jpg"), u8"files from sub folders");
	assert_equal(true, contains(platform::select_folders(not_recursive, true), u8"raw"), u8"include folders");

	const auto recursive_png = df::item_selector(test_files_folder, true, u8"*.png");
	assert_equal(false, contains(platform::select_files(recursive_png, true), u8"Screws.CR2"),
	             u8"no files from sub folder");
	assert_equal(true, contains(platform::select_files(recursive_png, true), u8"Cube.png"), u8"files from sub folders");
	assert_equal(false, contains(platform::select_files(recursive_png, true), u8"Test.jpg"),
	             u8"files from sub folders");
	assert_equal(false, contains(platform::select_folders(recursive_png, true), u8"raw"), u8"include folders");

	const auto not_recursive_cr2 = df::item_selector(test_files_folder, false, u8"*.cr2");
	assert_equal(false, contains(platform::select_files(not_recursive_cr2, true), u8"Screws.CR2"),
	             u8"no files from sub folder");
	assert_equal(false, contains(platform::select_files(not_recursive_cr2, true), u8"Cube.png"),
	             u8"files from sub folders");
	assert_equal(false, contains(platform::select_files(not_recursive_cr2, true), u8"Test.jpg"),
	             u8"files from sub folders");
	assert_equal(true, contains(platform::select_folders(not_recursive_cr2, true), u8"raw"), u8"include folders");

	const auto recursive_cr2 = df::item_selector(test_files_folder, true, u8"*.cr2");
	assert_equal(true, contains(platform::select_files(recursive_cr2, true), u8"Screws.CR2"),
	             u8"no files from sub folder");
	assert_equal(false, contains(platform::select_files(recursive_cr2, true), u8"Cube.png"),
	             u8"files from sub folders");
	assert_equal(false, contains(platform::select_files(recursive_cr2, true), u8"Test.jpg"),
	             u8"files from sub folders");
	assert_equal(false, contains(platform::select_folders(recursive_cr2, true), u8"raw"), u8"include folders");
}
...
static void should_match_related(test_view::shared_test_context& stc)
{
	...
	assert_equal(1, count_search_results(stc.index, df::search_t().related(r)), u8"Related");
}
...
static void should_select_items()
{
	...
	assert_equal(expected_cached_item_count + 2_z, s.search_items().items().size(), u8"items loaded into state");
	assert_equal(1_z, s.search_items().folders().size(), u8"folders loaded into state");

	assert_equal(0_z, s.selected_count(), u8"invalid selection");

	s.select(view, find_item_n(s, 3), false, false, false);
	s.update_selection();
	assert_equal(1_z, s.selected_count(), u8"invalid selection");
	...
	s.update_selection();
	assert_equal(2_z, s.selected_count(), u8"invalid control selection");
	...
	s.update_selection();
	assert_equal(4_z, s.selected_count(), u8"invalid selection");

	s.select(view, find_item_n(s, 2), true, false, false);
	s.update_selection();
	assert_equal(3_z, s.selected_count(), u8"invalid selection");
...
{
	const df::file_path src_path1(test_files_folder, u8"Test.jpg");
	const df::file_path src_path2(test_files_folder, u8"Gherkin.CR2");

	const auto save_path_1 = _temps.next_path(u8".jpg");
	const auto save_path_2 = _temps.next_path(u8".cr2");
	...
	s.item_index.scan_items(s.search_items(), false, false, false, false, test_token);
	assert_equal(3, s.displayed_rating(), u8"invalid jpeg rating");

	s.toggle_rating(results, s.selected_items().items(), 0, view);
	s.item_index.scan_items(s.search_items(), false, false, false, false, test_token);
	assert_equal(0, s.displayed_rating(), u8"invalid remove jpeg rating");
	...
	s.item_index.scan_items(s.search_items(), false, false, false, false, test_token);
	assert_equal(4, s.displayed_rating(), u8"invalid cr2 rating");

	s.item_index.scan_items(s.search_items(), true, false, false, false, test_token);
	assert_equal(4, s.displayed_rating(), u8"invalid cr2 rating after thumbnail load");

	s.toggle_rating(results, s.selected_items().items(), 0, view);
	s.item_index.scan_items(s.search_items(), false, false, false, false, test_token);
	assert_equal(0, s.displayed_rating(), u8"invalid remove cr2 rating");
}
...
static void should_enable_based_on_selection()
{
	...
	assert_equal(0_z, s.selected_count(), u8"by default nothing selected");
	assert_can_process(s, false, false, false, false, false, u8"nothing selected");

	s.select(view, u8"test.jpg", false);
	s.update_selection();

	assert_equal(1_z, s.selected_count(), u8"select test.jpg");
	assert_can_process(s, true, true, true, true, true, u8"photo selected");

	s.select(view, u8"gizmo.mp4", false);
	s.update_selection();

	assert_equal(1_z, s.selected_count(), u8"select gizmo.mp4");
	assert_can_process(s, false, false, true, true, true, u8"mp4 selected");

	s.select(view, u8"Gherkin.xmp", false);
	s.update_selection();

	assert_equal(1_z, s.selected_count(), u8"Gherkin.xmp");
	assert_can_process(s, false, false, true, true, true, u8"xmp selected");

	s.select(view, u8"test.jpg", false);
	s.select(view, u8"gizmo.mp4", true);
	s.update_selection();

	assert_equal(2_z, s.selected_count(), u8"test.jpg and gizmo.mp4");
	assert_can_process(s, false, false, true, true, true, u8"jpg and mp4 selected");

	s.select(view, u8"raw", false);
	s.update_selection();

	assert_equal(1_z, s.selected_count(), u8"folder");
	assert_can_process(s, false, false, false, false, true, u8"folder selected");
}

static void should_parse_search()
{
	const auto* const message = u8"Should tokenize search";
	search_tokenizer t;
	const auto parts = t.parse(
		u8"  (#tree or house)    -nega-tive aperture:f/2.0   's\"om:e#inv-lid~s' -description:\"hello world\" tag : spaces c:\\test 12:00 or -#tree|ant");
	...
	assert_equal(parts[0].scope, u8"tag", message);
	assert_equal(parts[0].term, u8"tree", message);
	...
	assert_equal(parts[1].term, u8"house", message);
	...
	assert_equal(parts[2].term, u8"nega-tive", message);
	...
	assert_equal(parts[3].scope, u8"aperture", message);
	assert_equal(parts[3].term, u8"f/2.0", message);

	assert_equal(parts[4].modifier.positive, true, message);
	assert_equal(parts[4].scope.empty(), true, message);
	assert_equal(parts[4].term, u8"s\"om:e#inv-lid~s", message);

	assert_equal(parts[5].modifier.positive, false, message);
	assert_equal(parts[5].scope, u8"description", message);
	assert_equal(parts[5].term, u8"hello world", message);

	assert_equal(parts[6].scope, u8"tag", message);
	assert_equal(parts[6].term, u8"spaces", message);

	assert_equal(parts[7].modifier.positive, true, message);
	assert_equal(parts[7].scope.empty(), true, message);
	assert_equal(parts[7].term, u8"c:\\test", message);

	assert_equal(parts[8].modifier.positive, true, message);
	assert_equal(parts[8].scope.empty(), true, message);
	assert_equal(parts[8].term, u8"12:00", message);
	...
	assert_equal(parts[9].scope, u8"tag", message);
	assert_equal(parts[9].term, u8"tree", message);
	...
	assert_equal(parts[10].term, u8"ant", message);
}
...
{
	d.next_date(false);
	assert_equal(expected_prev, d.text(), u8"prev date");
	d.next_date(true);
	assert_equal(expected_next, d.text(), u8"next date");
}

static void should_next_date_search()
{
	assert_date_shift(df::search_t().day(25, 5, 1972), u8"1972-may-24", u8"1972-may-25");
	assert_date_shift(df::search_t().day(1, 1, 2020), u8"2019-dec-31", u8"2020-jan-1");
	assert_date_shift(df::search_t().day(1, 1, 0), u8"dec-31", u8"jan-1");
	assert_date_shift(df::search_t().year(2010), u8"2009", u8"2010");
	assert_date_shift(df::search_t().month(1), u8"December", u8"January");
	assert_date_shift(df::search_t().month(1).year(2010), u8"2009-dec", u8"2010-jan");

	assert_date_shift(df::search_t().day(1, 1, 2020, df::date_parts_prop::modified), u8"modified:2019-dec-31",
	                  u8"modified:2020-jan-1");
	assert_date_shift(df::search_t().day(1, 1, 0, df::date_parts_prop::modified), u8"modified:dec-31",
	                  u8"modified:jan-1");
	assert_date_shift(df::search_t().year(2010, df::date_parts_prop::created), u8"created:2009", u8"created:2010");
	assert_date_shift(df::search_t().month(1, df::date_parts_prop::created).year(2010, df::date_parts_prop::created),
	                  u8"created:2009-dec", u8"created:2010-jan");
}
...
static void should_parse_search_input()
{
	...
	assert_equal(test_files_folder.parent().text(), s.search().text(), u8"Parse ..");

	s.open(view, test_files_folder.text());
	s.open(view, u8"**");

	assert_equal(base_folder + u8"\\**", s.search().text(), u8"Parse **");
}

static void should_parent()
{
	const auto folder = df::folder_path(u8"c:\\windows\\system32");
	assert_equal(u8"c:\\windows", folder.parent().text(), u8"parent test");
	assert_equal(u8"c:\\", folder.parent().parent().text(), u8"parent test");
	assert_equal(u8"c:\\", folder.parent().parent().parent().text(), u8"parent test");
	...
	s.open(view, base_folder + u8" test");
	assert_equal(base_folder, s.parent_search().parent.text(), u8"parent test");

	s.open(view, base_folder + u8"\\raw");
	const auto raw_parent = s.parent_search();
	assert_equal(base_folder, raw_parent.parent.text(), u8"parent \\raw");
	assert_equal(u8"raw", raw_parent.name, u8"parent name \\raw");

	s.open(view, base_folder + u8"\\*.png");
	assert_equal(base_folder, s.parent_search().parent.text(), u8"parent *.png");

	s.open(view, base_folder + u8" @photo");
	assert_equal(base_folder, s.parent_search().parent.text(), u8"parent @photo");

	s.open(view, u8"@photo");
	assert_equal({}, s.parent_search().parent.text(), u8"parent @photo");

	s.open(view, u8"@photo");
	s.update_item_groups();
	s.select_next(view, true, false, false);
	s.update_selection();
	assert_equal(base_folder, s.parent_search().parent.text(), u8"parent @photo with selection");

	// DATES
	s.open(view, u8"1972-may-25");
	assert_equal(u8"1972-may", s.parent_search().parent.text(), u8"parent");

	s.open(view, u8"may-25");
	assert_equal(u8"May", s.parent_search().parent.text(), u8"parent");

	s.open(view, u8"1972-may");
	assert_equal(u8"1972", s.parent_search().parent.text(), u8"parent");

	s.open(view, u8"2009 December");
	assert_equal(u8"2009", s.parent_search().parent.text(), u8"parent");
}
...
static void should_escape()
{
	...
	// TODO more things
	s.view_mode(view_type::items);
	s.open(view, base_folder + u8"\\**");
	s.escape(view);
	assert_equal(base_folder, s.search().text(), u8"parent escape **");
}


static void should_calc_HMACSHA1()
{
	const auto signature = crypto::hmac_sha1(u8"Jefe", u8"what do ya want for nothing?");
	assert_equal(u8"7/zfauXrL6LSdBbV8YTfnCWafHk=", signature, u8"Signature");
}

static void should_calc_hashes()
{
	assert_equal(u8"A9993E364706816ABA3E25717850C26C9CD0D89D", crypto::to_sha1(u8"abc"), u8"SHA1");
	assert_equal(u8"187797D630ECAA0FC1B920CD9F809C2BBFFCBF4C", crypto::to_sha1(long_text), u8"SHA1");
	assert_equal(u8"BA7816BF8F01CFEA414140DE5DAE2223B00361A396177A9CB410FF61F20015AD", crypto::to_sha256(u8"abc"), u8"SHA256");
	assert_equal(u8"1660F10AEC042D762CF8B1C53E976F890C8E797BEF74807F505EDCE20308FC2F", crypto::to_sha256(long_text), u8"SHA256");

	const auto crc_data = u8"hello world"s;
	const auto crc_result = crypto::crc32c(crc_data.data(), crc_data.size());
	assert_equal(0xc99465aa, crc_result, u8"crc32");

	const auto crc_c = ~calc_crc32c_c(crypto::CRCINIT, crc_data.data(), crc_data.size());
	assert_equal(0xc99465aa, crc_c, u8"crc32 c");

	if (platform::sse2_supported)
	{
		const auto crc_sse2 = ~calc_crc32c_sse2(crypto::CRCINIT, crc_data.data(), crc_data.size());
		assert_equal(0xc99465aa, crc_sse2, u8"crc32 sse");
	}

	if (platform::neon_supported)
	{
		const auto crc_neon = ~calc_crc32c_arm(crypto::CRCINIT, crc_data.data(), crc_data.size());
		assert_equal(0xc99465aa, crc_neon, u8"crc32 neon");
	}
}
...
static void should_convert_utf8()
{
	...
	for (auto i = 0; i < 5; i++)
	{
		stars[i] = static_cast<uint16_t>(i & 0x01 ? icon_index::star_solid : icon_index::star);
	}
	stars[5] = 0;

	std::wstring_view strings[] = {
		L"In vollen Zügen genießen",
		L"Nældens takvinge",
		L"Žižkov",
		L"Japanese こんにちは世界",
		L"مرحبا العالم  Arabic",
		stars
	};

	for (const auto src : strings)
	{
		assert_equal(src, str::utf8_to_utf16(platform::utf16_to_utf8(src)), u8"to utf8 and back");
		assert_equal(platform::utf16_to_utf8(src), str::utf16_to_utf8(src), u8"to utf8");
	}

	const wchar_t icon_text[2] = {static_cast<wchar_t>(icon_index::fit), 0};
	const auto icon_text_converted = str::utf8_to_utf16(str::utf16_to_utf8(icon_text));
	assert_equal(icon_text, icon_text_converted, u8"icon to utf8");
}

static void should_split()
{
	const auto* const to_be_split = u8"H:\\2-Archief VIDEO privé\\Eigen video's\nF:\\1-Archief FOTOGRAFIE privé";
	...
	assert_equal(u8"bbb ccc", parts2[1], u8"Split 2");
	assert_equal(u8"ddd", parts2[2], u8"Split 3");
	assert_equal(u8"ee ff ", parts2[3], u8"Split 4");
...
{
	assert_equal(false, str::is_wildcard(u8""));
	assert_equal(false, str::is_wildcard(u8"abcdef"));
	assert_equal(true, str::is_wildcard(u8"abc*"));
	assert_equal(false, str::is_wildcard(u8"abc\\*"));
	assert_equal(false, str::is_wildcard(u8"abc\\*ef"));
}
...
static void should_find_closest_location()
{
	...
	assert_equal(u8"St Paul's", locations.find_closest(51.5142, -000.0985).place, u8"City");
	assert_equal(u8"Armidale", locations.find_closest(-30.515, 151.665).place, u8"City");
	assert_equal(u8"Johannesburg", locations.find_closest(-26.204444, 28.045556).place, u8"City");
	assert_equal(u8"Santiago", locations.find_closest(-33.45, -70.666667).place, u8"City");
	assert_equal(u8"Eastern Parkway", locations.find_closest(40.664167, -73.938611).place, u8"City");
	assert_equal(u8"Beijing", locations.find_closest(39.913889, 116.391667).place, u8"City");
}
...
static void should_find_location()
{
	...
	assert_equal(u8"City of London", locations.find_by_id(2643741).place, u8"City");
	assert_equal(u8"King of Prussia", locations.find_by_id(5196220).place, u8"City");

	assert_equal(u8"London, England, United Kingdom",
	             locations.auto_complete(u8"london", 8, default_location)[0].location.str(), u8"City");
	assert_equal(u8"Londonderry Station, Nova Scotia, Canada",
	             locations.auto_complete(u8"london canada", 8, default_location)[0].location.str(), u8"City");
	assert_equal(u8"Armidale, New South Wales, Australia",
	             locations.auto_complete(u8"armid aust", 8, default_location)[0].location.str(), u8"City");
	assert_equal(u8"Birmingham, England, United Kingdom",
	             locations.auto_complete(u8"birm gb", 8, default_location)[0].location.str(), u8"City");
	assert_equal(u8"King of Prussia, Pennsylvania, United States",
	             locations.auto_complete(u8"king pru usa", 8, default_location)[0].location.str(), u8"City");
}

static void should_detect_original_path()
{
	const df::file_path path(u8"c:\\temp\\test.original.jpg");
	assert_equal(true, path.is_original(), u8"detect original");
}
...
static void should_not_reload_thumb_when_valid()
{
	...
	assert_equal(true, i_local->should_load_thumbnail(), u8"should load after db load");

	i_local->thumbnail(loaded.i);
	assert_equal(true, i_local->should_load_thumbnail(), u8"should load without timestamp");
...
void should_copy_preserve_properties()
{
	...
	assert_equal(19, ps_copy.read_int(prop::media_position), u8"should copy media_position");*/
}
...
static void should_reload_thumb_after_scan()
{
	...
	auto path_test = df::file_path(test_files_folder, u8"Test.jpg");
	auto path_sony = df::file_path(test_files_folder, u8"Sony.jpg");

	const auto test_item = load_item(index, path_test, false);
	const auto sony_item = load_item(index, path_sony, false);

	assert_equal(false, test_item->should_load_thumbnail(), u8"should_load_thumbnail for test.jpg");
	assert_equal(false, sony_item->should_load_thumbnail(), u8"should_load_thumbnail for sony.jpg");
	...
	assert_equal(true, test_item->should_load_thumbnail(),
	             u8"should_load_thumbnail for test.jpg after db load_thumbnails");
	assert_equal(true, sony_item->should_load_thumbnail(),
	             u8"should_load_thumbnail for sony.jpg after db load_thumbnails");

	index.scan_items(items, true, false, false, false, test_token);
	db.perform_writes();

	assert_equal(false, test_item->should_load_thumbnail(),
	             u8"should_load_thumbnail for test.jpg after index load_thumbnails");
	assert_equal(false, sony_item->should_load_thumbnail(),
	             u8"should_load_thumbnail for sony.jpg after index load_thumbnails");
}
...
static void should_rename()
{
	...
	const df::file_path src_path(test_files_folder, u8"Test.jpg");
	const auto save_path_1 = _temps.next_path(u8".jpg");
	const auto save_path_2 = _temps.next_path(u8".jpg");

	platform::copy_file(src_path, save_path_1, false, false);

	auto test_item = load_item(index, save_path_1, false);

	assert_equal(true, test_item->rename(index, save_path_2.file_name_without_extension()).success(), u8"can rename");
	assert_equal(save_path_2.name(), test_item->name(), u8"renamed");
	assert_equal(true, save_path_2.exists(), u8"renamed exists");
...
static void should_not_overwrite_during_rename()
{
	...
	assert_equal(false, test_item->rename(index, save_path_2.file_name_without_extension()).success(),
	             u8"should not rename");
	assert_equal(save_path_1.name(), test_item->name(), u8"not renamed");
	assert_equal(true, test_item->path().exists(), u8"exists");
}
...
static void should_encrypt_password()
{
	...
	assert_equal(base64_encode(test_enc), base64_encode(crypto::encrypt(test_dec, test_key)), u8"encrypt using aes");
	...
	static constexpr std::u8string_view password = u8"diffractor-hello";

	for (const auto& val : test_values)
	{
		auto result = crypto::decrypt(crypto::encrypt(val, password), password);
		assert_equal(val, std::u8string(result.begin(), result.end()), u8"Encode - Decode");
	}
...
static void should_persist_to_registry()
{
	...
	{
		std::u8string actual;
		archive->write({}, u8"test", expected);
		archive->read({}, u8"test", actual);

		assert_equal_strict(expected, actual, u8"Persist To Registry");
	}
}
...
static void should_detect_duplicates(test_view::shared_test_context& stc)
{
	...
	const auto path1 = df::file_path(test_files_folder, u8"Test.jpg");
	const auto path2 = df::file_path(test_files_folder, u8"Test90.jpg");
	const auto path3 = df::file_path(test_files_folder, u8"Test180.jpg");
	const auto path4 = df::file_path(test_files_folder, u8"Test270.jpg");
	...
	assert_equal(1u, index.find_item(test_item1->path()).duplicates.count, u8"duplicates");
	assert_equal(1u, index.find_item(test_item2->path()).duplicates.count, u8"duplicates");
	assert_equal(1u, index.find_item(test_item3->path()).duplicates.count, u8"duplicates");
	assert_equal(1u, index.find_item(sony_item->path()).duplicates.count, u8"duplicates");
}
...
static void should_record_crashes()
{
	...
	const auto paths = {
		test_files_folder.combine_file(u8"Test.jpg"),
		test_files_folder.combine_file(u8"Test90.jpg"),
		test_files_folder.combine_file(u8"Small.jpg"),
		test_files_folder.combine_file(u8"Lossless0.jpg")
	};
	...
	{
		{
			...
			assert_equal(test_crash_files.is_known_crash_file(path), false, u8"is_known_crash_file");
		...
		{
			crash_files_db crash_files(db_path);

			assert_equal(crash_files.is_known_crash_file(path), true, u8"is_known_crash_file");
		}
	}
}
...
//
//	const auto short_text = u8"Hide/Show Thumbnails\t";
...
//
//		assert_equal(extent_text.cy, extent_text2.cy, str::format(u8"short text height at width {} ({} / {})", cx, extent_text.cx, extent_text2.cx).c_str());
//	}
//
...
//
//		assert_equal(extent_text.cy, extent_text2.cy, str::format(u8"long text height at width {} ({} / {})", cx, extent_text.cx, extent_text2.cx).c_str());
//	}
//}
...
static void should_parse_command_line()
{
	...
	assert_equal(true, cl1.no_gpu, u8"no_gpu");
	assert_equal(false, cl1.no_indexing, u8"no_indexing");

	command_line_t cl2;
	cl2.parse(test_files_folder.text());

	assert_equal(false, cl2.folder_path.is_empty(), u8"folder_path");
	assert_equal(std::u8string_view{}, cl2.selection.name(), u8"selection name");
	assert_equal(test_files_folder.text(), cl2.folder_path.folder().text(), u8"folder path");
	assert_equal(false, cl2.no_gpu, u8"no_gpu");
	assert_equal(false, cl2.no_indexing, u8"no_indexing");


	const auto path3 = test_files_folder.combine_file(u8"test.jpg");
	command_line_t cl3;
	cl3.parse(str::format(u8"{} -no-indexing", path3));

	assert_equal(path3.folder().text(), cl3.folder_path.folder().text(), u8"folder_path");
	assert_equal(path3.name(), cl3.selection.name(), u8"selection name");
	assert_equal(path3.folder().text(), cl3.selection.folder().text(), u8"selection folder");
	assert_equal(false, cl3.no_gpu, u8"no_gpu");
	assert_equal(true, cl3.no_indexing, u8"no_indexing");

	command_line_t cl4;
	cl4.parse(u8"--no-gpu \"C:\\Program Files\"");
	assert_equal(true, cl4.no_gpu, u8"no_gpu");
	assert_equal(false, cl4.folder_path.is_empty(), u8"folder_path program Files");

	command_line_t cl5;
	cl5.parse(u8"----- --no-gpu");
	assert_equal(true, cl5.no_gpu, u8"no_gpu");
}

static void should_trim_strings()
{
	assert_equal(u8"xxx", str::trim_and_cache(u8"xxx\n"), u8"remove cr lf");
	assert_equal(u8"xxx", str::trim_and_cache(u8"\rxxx\r"), u8"remove lf");
	assert_equal(u8"xxx", str::trim_and_cache(u8"   xxx\t\t "), u8"remove space");
}


static void should_format_text()
{
	assert_equal(u8"ac-dc", str::format(u8"{2}{0}-{1}{0}", u8"c", u8"d", u8"a"), u8"order");
	assert_equal(u8"0.00123", str::format(u8"{}", 0.00123), u8"double");
	assert_equal(u8"0.001", str::format(u8"{:0.3}", 0.00123), u8"double");
	assert_equal(u8"5.5", str::format(u8"{}", 5.5000), u8"double");
	assert_equal(u8"123", str::format(u8"{}", 123), u8"int");
	assert_equal(u8"0123", str::format(u8"{:04}", 123), u8"int");
	assert_equal(u8" 123", str::format(u8"{:4}", 123), u8"int");
	assert_equal(u8"hex=7B", str::format(u8"hex={:x}", 0x7B), u8"hex");
	assert_equal(u8"-test-", str::format(u8"-{}-", u8"test"), u8"char8_t*");
	assert_equal(u8"-test-", str::format(u8"-{}-", std::u8string(u8"test")), u8"string");
	assert_equal(u8"-test-", str::format(u8"-{}-", std::u8string_view(u8"test")), u8"string_view");
	assert_equal(u8"33 {} {test}", str::format(u8"{} {{}} {{test}}", 33), u8"string_view");
	assert_equal(u8"22 x 33", str::format(u8"{} x {}", 22, 33), u8"string_view");
}
...
{
	assert_equal(u8"*white* on blond", find_and_format_result(u8"white on blond", u8"white"));
	assert_equal(u8"*whi*te on *bl*ond", find_and_format_result(u8"white on blond", u8"whi bl"));
	assert_equal(u8"*white* on *blond*", find_and_format_result(u8"white on blond", u8"white blond"));
	assert_equal(u8"*white* bl on *blond*", find_and_format_result(u8"white bl on blond", u8"white blond"));
}


static void should_scan_info_from_title()
{
	const auto ps1 = scan_info_from_title(u8"Game.of.Thrones.S02E06.HDTV.x264 - 2HD.mp4");

	assert_equal(2, ps1.season, u8"season");
	assert_equal(6, ps1.episode, u8"episode");
	assert_equal(u8"Game of Thrones", ps1.show, u8"show");

	const auto ps2 = scan_info_from_title(u8"It's.a.Wonderful.Life.1946.720p.BluRay.x264.YIFY.mp4");

	assert_equal(1946, ps2.year, u8"year");
	assert_equal(u8"It's a Wonderful Life", ps2.title, u8"title");
}

static void should_compare_versions()
{
	const df::version current_version(s_app_version);
	assert_equal(s_app_version, current_version.to_string(), u8"Can parse and to_string current version");

	const df::version test_version1(u8"123.45");
	const df::version test_version1b(u8"123.45");
	df::version test_version2(u8"456.1");

	assert_equal(u8"123.45", test_version1.to_string(), u8"Can parse and to_string test version 1");
	assert_equal(u8"456.1", test_version2.to_string(), u8"Can parse and to_string test version 2");

	assert_equal(true, test_version1 < test_version2, u8"Less op version");
	assert_equal(false, test_version2 < test_version1, u8"Less op version");
	assert_equal(false, test_version1 == test_version2, u8"== op version");
	assert_equal(true, test_version1 == test_version1b, u8"== op version");

	assert_equal(u8"457.1", (test_version2 + 1).to_string(), u8"+ op version");
}
...
void test_view::register_tests()
{
	...
	// Infra
	//
	register_test(u8"Should calc HMAC SHA1", should_calc_HMACSHA1);
	register_test(u8"Should calc Hashes", should_calc_hashes);
	register_test(u8"Should convert Utf8", should_convert_utf8);
	register_test(u8"Should split", should_split);
	register_test(u8"Should detect wildcard", should_detect_wildcard);
	register_test(u8"Should handle international characters", should_handle_international_characters);
	register_test(u8"Should reset dx11", [this]
	{
		_state.queue_ui([this]
		{
	...
	//register_test(u8"Should measure text consistently", should_measure_text_consistently);
	register_test(u8"Should record crashes", should_record_crashes);
	register_test(u8"Should compare versions", should_compare_versions);
	register_test(u8"Should parse facebook Json", should_parse_facebook_json);
	register_test(u8"Should Encrypt Password", should_encrypt_password);
	register_test(u8"Should persist strings in registry", should_persist_to_registry);
	register_test(u8"Should parse selector", should_parse_selector);
	register_test(u8"Should select files", should_select_files);
	register_test(u8"Should parent", should_parent);
	register_test(u8"Should escape", should_escape);
	register_test(u8"Should scan info from title", should_scan_info_from_title);
	register_test(u8"Should parse command line", should_parse_command_line);
	register_test(u8"Should trim strings", should_trim_strings);
	register_test(u8"Should format text", should_format_text);
	register_test(u8"Should find text", should_find_text);
	register_test(u8"Should find Location", should_find_location);

	//
	// Scan Metadata
	//
	register_test(u8"Should scan jpg metadata", should_scan_jpeg);
	register_test(u8"Should scan avi metadata", should_scan_avi);
	register_test(u8"Should scan mov metadata", should_scan_mov);
	register_test(u8"Should scan mp3 metadata", should_scan_mp3);
	register_test(u8"Should scan mp4 metadata", should_scan_mp4);
	register_test(u8"Should scan raw metadata", should_scan_raw);
	register_test(u8"Should parse Xmp", should_parse_xmp);
	register_test(u8"Should merge Xmp sidecar", should_merge_xmp_sidecar);
	register_test(u8"Should replace tokens", should_replace_tokens);
	...
	{
		register_test(str::format(u8"Should update metadata {}", name), [name] { should_update_metadata(name); });
		register_test(str::format(u8"Should update location {}", name), [name] { should_update_location(name); });
		register_test(str::format(u8"Should update rating {}", name), [name] { should_update_rating(name); });
		register_test(str::format(u8"Should update tags {}", name), [name] { should_add_remove_tags(name); });
	}

	register_test(u8"Should update gps in exif", should_update_gps_in_exif);

	// 
	// Bitmap Edit
	//

	register_test(u8"Should preserve orientation", should_preserve_orientation);
	register_test(u8"Should resize", should_resize);
	register_test(u8"Should rotate", should_rotate);
	register_test(u8"Should rotate 133", should_rotate133);
	register_test(u8"Should rotate lossless", should_rotate_lossless);
	register_test(u8"Should save .png", [] { should_save(u8".png", true); });
	register_test(u8"Should save .jpg", [] { should_save(u8".jpg", true); });
	register_test(u8"Should save .webp", [] { should_save(u8".webp", true); });
	register_test(u8"Should convert raw to jpeg", should_convert_raw_to_jpeg);

	//
	// UI
	//
	register_test(u8"Should select correctly", should_select_items);
	register_test(u8"Should Enable based on selection", should_enable_based_on_selection);
	register_test(u8"Should update exif rating", should_update_exif_rating);
	register_test(u8"Should update formatted description", should_update_formatted_text);
	register_test(u8"Should toggle rating", should_toggle_rating);


	//
	// Index
	//
	register_test(u8"Should index", should_index);
	register_test(u8"Should store thumbnails", should_store_thumbnails);
	register_test(u8"Should store item properties", should_store_item_properties);
	register_test(u8"Should store pack properties", should_pack_item_properties);
	register_test(u8"Should store webservice results", should_store_webservice_results);
	register_test(u8"Should detect duplicates", should_detect_duplicates);
	register_test(u8"Should Rename", should_rename);
	register_test(u8"Should Rename with substitutions", should_rename_with_substitutions);
	register_test(u8"Should not overwrite during rename", should_not_overwrite_during_rename);
	register_test(u8"Should detect original path", should_detect_original_path);
	register_test(u8"Should not reload thumb when valid", should_not_reload_thumb_when_valid);
	register_test(u8"Should reload thumb after scan", should_reload_thumb_after_scan);
	register_test(u8"Should copy preserve properties", should_copy_preserve_properties);
	register_test(u8"Should detect rotation", should_detect_rotation);


	//
	// Search
	//
	register_test(u8"Should parse search", should_parse_search);
	register_test(u8"Should parse search input", should_parse_search_input);
	register_test(u8"Should parse searches", should_parse_searches);
	register_test(u8"Should next date search", should_next_date_search);
	...
	register_assert_format(df::search_t().with(u8"!-"));
	...
	register_assert_format(df::search_t().with(prop::tag, u8"tag with space").without(prop::tag, u8"space tag"));
	...
	register_assert_format(df::search_t().with_extension(u8"jpg"));

	auto register_assert_parse = [this](const std::u8string& query, const std::u8string& expected = {})
	{
		register_test(str::format(u8"Should parse {}", query), [query, expected](shared_test_context& stc)
		{
			const auto formatted = df::search_t::parse(query).format_terms();
			assert_equal(str::is_empty(expected) ? query : expected, formatted, u8"parse query");
			const auto parsed = df::search_t::parse(query).format_terms();
			assert_equal(formatted, parsed, u8"parse formatted");
		});
	};

	register_assert_parse(u8"2014 2015");
	register_assert_parse(u8"2014 @photo");
	register_assert_parse(u8"2014 -@video");
	register_assert_parse(u8"2014 @audio");
	register_assert_parse(u8"2014 or 2015");
	register_assert_parse(u8"Jana Alfie");
	register_assert_parse(u8"Jana or Alfie");
	register_assert_parse(u8"Jana -Alfie");
	register_assert_parse(u8"genre: Rock -artist: \"Counting Crows\"");
	register_assert_parse(u8"Created:7");
	register_assert_parse(u8"Modified:7");
	register_assert_parse(u8"(2014 or 2015) (May or June)");
	register_assert_parse(u8"Bertie or ((Jana or Alfie) or Amalka)");
	register_assert_parse(u8"(2013 or 2014 or 2015) (May or June or July)");
	register_assert_parse(u8"Rating:5", u8"rating: 5");
	register_assert_parse(u8"dec-25");
	register_assert_parse(u8"2020-aug-16");
	register_assert_parse(u8"2020-aug");
	register_assert_parse(u8"modified:2020-aug");
	register_assert_parse(u8"modified:2020-aug-16");
	register_assert_parse(u8"modified:aug-16");
	register_assert_parse(u8"@duplicates");
	register_assert_parse(u8"loc:-30.515+151.66+5");
	register_assert_parse(u8"size:1mb", u8"size: 1 MB");
	register_assert_parse(u8"> size:1mb", u8"> size: 1 MB");
	register_assert_parse(u8"ext:jpg");
	register_assert_parse(u8"with:tag", u8"with: tag");
	register_assert_parse(u8"without:tag", u8"without: tag");
	register_assert_parse(u8"c:\\windows");
	register_assert_parse(u8"c:\\windows");
	register_assert_parse(u8"c:\\windows with:tag", u8"c:\\windows with: tag");
	register_assert_parse(u8"c:\\windows without:tag", u8"c:\\windows without: tag");
	register_assert_parse(u8"@ photo", u8"@photo");
	register_assert_parse(u8"# key1", u8"#key1");
	register_assert_parse(u8"# \"tag with space\"", u8"#\"tag with space\"");
	register_assert_parse(u8"#'tag with space'", u8"#\"tag with space\"");

	register_test(u8"Should match terms", should_match_terms);
	register_test(u8"Should match related", should_match_related);
	register_test(u8"Should not match folder without ", should_not_match_folder_without);
	register_test(u8"Should match date", [] { should_match_date(u8"2012-09-14", df::date_t(2012, 9, 14)); });
	register_test(u8"Should match date", [] { should_match_date(u8"2012", df::date_t(2012, 1, 14)); });
	register_test(u8"Should match date", [] { should_match_date(u8"2012|2013", df::date_t(2012, 1, 14)); });
	register_test(u8"Should match date", []
	{
		should_match_date(u8"(April or June) (2013 or 2015)", df::date_t(2013, 4, 22));
	});
	register_test(u8"Should match date", []
	{
		should_match_date(u8"(April or June) (2013 or 2015)", df::date_t(2015, 6, 7));
	});
	register_test(u8"Should match date", [] { should_match_date(u8"age:4", df::date_t(1999, 12, 30)); });

	auto register_should_search = [this](const std::u8string_view query, int expected)
	{
		register_test(str::format(u8"Should search {}", query), [query, expected](shared_test_context& stc)
		{
			stc.lazy_load_index();
	...
	register_should_search(u8"2012-09-14", 5);
	register_should_search(u8"Created:2012-09-14", 5);
	register_should_search(u8"2010-05-25", 0);
	register_should_search(u8"2010-5-25", 0);
	register_should_search(u8"Created:2010-05-25", 0);
	register_should_search(u8"2009-11-15", 1);

	register_should_search(u8"@video", 4);
	register_should_search(u8"@audio", 4);
	register_should_search(u8"@photo", 26);

	register_should_search(u8"@ photo", 26);
	register_should_search(u8"@   photo", 26);

	register_should_search(u8"key1", 4);
	register_should_search(u8"Tag:key1", 4);
	register_should_search(u8"Tag :key1", 4);
	register_should_search(u8"Tag: key1", 4);
	register_should_search(u8"#key1", 4);
	register_should_search(u8"# key1", 4);
	register_should_search(u8"Tag:dog Tag:london", 1);
	register_should_search(u8"not_exist", 0);
	register_should_search(u8"Tag:not_exist", 0);
	register_should_search(u8"#ke*", 4);
	register_should_search(u8"ke*", 5);

	register_should_search(u8"Megapixels:1.6 dog", 1);
	register_should_search(u8"Megapixels:1.6", 2);
	register_should_search(u8"(< Megapixels:1.0 > Megapixels:0.5)", 7);
	register_should_search(u8"Megapixels:2", 1);
	register_should_search(u8"pixels:2", 1);
	register_should_search(u8"> pixels:1", 8);
	register_should_search(u8">pixels:1", 8);
	register_should_search(u8">pixels :1", 8);
	register_should_search(u8">pixels: 1", 8);
	register_should_search(u8"> pixels : 1", 8);
	register_should_search(u8"2mp", 1);
	register_should_search(u8"6000x4000", 1);

	register_should_search(u8"Aperture:f/5", 1);
	register_should_search(u8"f/3.5", 6);
	register_should_search(u8"f/1.8", 0);
	register_should_search(u8"f/4.0", 0);
	register_should_search(u8"f/6.3", 5);

	register_should_search(u8"with:Exposure @photo", 14);
	register_should_search(u8"with: Exposure @ photo", 14);
	register_should_search(u8"without:Exposure @photo", 12);
	register_should_search(u8"without:Exposure", 22);
	register_should_search(u8"with:Exposure", 15);
	register_should_search(u8"with: Exposure", 15);
	register_should_search(u8"ExposureTime:1/20s", 1);
	register_should_search(u8"ExposureTime: 1/20s", 1);
	register_should_search(u8"1/20s", 1);
	register_should_search(u8"1/100s", 5);
	register_should_search(u8"1/1000s", 0);

	register_should_search(u8"iso400 @photo", 2);
	register_should_search(u8"iso:400 @photo", 2);
	register_should_search(u8"iso: 400 @photo", 2);
	register_should_search(u8"iso : 400 @photo", 2);
	register_should_search(u8">= iso:400 @photo", 3);

	register_should_search(u8"1:26", 1);
	register_should_search(u8"0:10", 5);
	register_should_search(u8"7:77", 0);
	register_should_search(u8"10:00", 0);

	register_should_search(u8"size:0.3mb", 4);
	register_should_search(u8"size:14kb", 1);
	register_should_search(u8"size:5.1mb", 1);
	register_should_search(u8">size:1mb", 8);

	register_should_search(u8"key1", 4);
	register_should_search(u8"dog london", 1);
	register_should_search(u8"prague", 5);
	register_should_search(u8"ipad", 1);
	register_should_search(u8"48kHz", 2);
	register_should_search(u8"44.1kHz", 4);
	register_should_search(u8"tag:dog tag:london", 1);
	register_should_search(u8"dog or london", 4);
	register_should_search(u8"Rock", 3);
	register_should_search(u8"canon @photo", 10);
	register_should_search(u8"nikon d100", 1);

	register_should_search(u8"ext:cr2", 2);
	register_should_search(u8"ext:.cr2", 2);
	...
	register_test(u8"Should Find Closest Location", should_find_closest_location);
	register_test(u8"Should not crash on JPEG", [] { should_not_crash(u8"small.jpg"); });
	register_test(u8"Should not crash on GIF", [] { should_not_crash(u8"tuesday.gif"); });
	register_test(u8"Should not crash on TIFF", [] { should_not_crash(u8"small.tif"); });
	register_test(u8"Should not crash on PNG", [] { should_not_crash(u8"cube.png"); });
	//register_test(u8"Should not crash on PSD", [] { should_not_crash(u8"cherrys.psd"); });
