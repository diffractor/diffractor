...
void rename_view::run()
{
	...
	const auto results = std::make_shared<view_command_status>(_state._async, cancel_source,
		...
		[view, processing_generation, detach](std::string message,
		                                      std::vector<view_operation_result> operation_results)
		{
			if (!view->is_processing_generation(processing_generation)) return;
			view->end_processing();
			const auto result_summary = view->show_results(operation_results);
			view->_status = !message.empty() ? std::move(message) : result_summary;
	...
	_state.queue_async(async_queue::work,
		[&s = _state, weak_view, sources, name_template, start_seq, policy, completion_status, generation]
		{
			...
			s.queue_ui([weak_view, renames = std::move(renames), completion_status, generation]() mutable
			{
				...
				view->_state.invalidate_view(view_invalid::view_layout | view_invalid::controller | view_invalid::status);
			});
		});
}
